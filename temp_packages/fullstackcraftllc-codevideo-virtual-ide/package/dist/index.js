import { isCourse, isLesson, isValidActions, isFileExplorerAction, isEditorAction, isTerminalAction, isAuthorAction, advancedCommandValueSeparator } from '@fullstackcraftllc/codevideo-types';
...
const supportedCommands = ["cat", "cd", "cp", "echo", "ls", "mkdir", "mv", "pwd", "touch", "tree"];
/**
 ...
 */
class VirtualIDE {
    constructor(project, initialActionIndex, verbose) {
        this.virtualEditors = [];
        this.virtualTerminals = [];
        this.currentEditorIndex = 0;
        this.currentTerminalIndex = 0;
        this.currentAuthorIndex = 0;
        this.currentCursorPosition = { x: -1, y: -1 }; // x is column, y is row - we allow both to be negative because a user may not want to have a cursor shown or used
        this.verbose = false;
        this.logs = [];
     ...
     */
    addVirtualEditor(fileName, virtualEditor) {
    ...
    applyAction(action) {
        if (isFileExplorerAction(action)) {
            if (this.verbose)
                console.log("VirtualIDE: Applying FILE EXPLORER ACTION", action);
            ...
            if (this.verbose)
                console.log("VirtualIDE: Applying EDITOR action", action);
            ...
            if (this.verbose)
                console.log("VirtualIDE: Applying TERMINAL action", action);
            ...
            if (this.verbose)
                console.log("VirtualIDE: Applying AUTHOR action", action);
            ...
            if (this.verbose)
                console.warn("VirtualIDE: Unknown action:", action);
        ...
        if (action.name === "editor-save") {
            ...
            if (this.verbose)
                console.log(`VirtualIDE: Saving file: ${filename} with contents: ${contents}`);
    ...
    executeTerminalCommandSideEffects() {
        if (this.verbose)
            console.log("VirtualIDE: Executing terminal command side effects");
        ...
        if (!supportedCommands.includes(commandName)) {
            if (this.verbose)
                console.log(`VirtualIDE: Unsupported command: ${commandName} - supported commands are: ${supportedCommands.join(", ")}`);
        ...
        if (parts.length === 1 && lastCommand === "ls") {
            ...
            if (this.verbose)
                console.log(`VirtualIDE: ls output: ${lsString}`);
        ...
        if (this.verbose)
            console.log(`VirtualIDE: Present working directory: ${pwd}`);
        ...
        if (parts.length == 2 && commandName === "touch") {
            if (this.verbose)
                console.log(`VirtualIDE: Creating file: ${parts[1]}`);
            ...
            if (this.verbose)
                console.log(`VirtualIDE: Creating file: ${fullFilePath}`);
        ...
        if (parts.length == 2 && commandName === "mkdir") {
            if (this.verbose)
                console.log(`VirtualIDE: Creating directory: ${parts[1]}`);
        ...
        if (parts.length == 2 && commandName === "cd") {
            if (this.verbose)
                console.log(`VirtualIDE: Changing directory to: ${parts[1]}`);
            ...
            if (targetDir === "..") {
                ...
                pwd = parts.join("/");
                if (this.verbose)
                    console.log(`VirtualIDE: Changing directory to: ${pwd}`);
        ...
        if (parts.length == 2 && commandName === "cat") {
            ...
            if (this.verbose)
                console.log(`VirtualIDE: Reading file: ${absoluteFilePath}`);
        ...
        if (parts.length == 3 && commandName === "cp") {
            ...
            if (this.verbose)
                console.log(`VirtualIDE: Copying file from: ${absoluteFromPath} to: ${absoluteToPath}`);
        ...
        if (parts.length == 3 && commandName === "mv") {
            ...
            const absoluteToPath = pwd + "/" + to;
            if (this.verbose)
                console.log(`VirtualIDE: Moving file from: ${absoluteFromPath} to: ${absoluteToPath}`);
        ...
        if (this.verbose)
            console.log(`VirtualIDE: Unknown command: ${commandName}`);
    ...
    reconstituteFromCourseAtActionIndex(course, actionIndex) {
    ...
    reconstituteFromLessonAtActionIndex(lesson, actionIndex) {
        if (actionIndex === undefined) {
            actionIndex = 0;
        }
    ...
    reconstituteFromActionsAtActionIndex(actions, actionIndex) {
        if (actionIndex === undefined) {
            actionIndex = 0;
        }
